...
export const register = async (req, res) => {
  try {
    ...
    await sendMail(
      email,
      "Please verify your account for Spandan App",
      
      `Place the below OTP on the Verify Screen Option in the App to get your profile verified within next 48 hours,else the account might get suspended"
      Your OTP is ${otp}
...
export const verify = async (req, res) => {
  try {
    ...
    sendToken(res, user, 200, "Account Verified for Spandan");
  } catch (err) {
    console.log(err);
    // res.status(500).json({success: false, msg:err.message})
  }
...
export const forgetPassword = async (req,res) =>{
  try{
        ...
        await sendMail(email , "Password Reset Request for Spandan App" , `OTP for resetting password:  ${otp}`)
